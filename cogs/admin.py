...
class Admin:
    ...
    async def debug(self, ctx):
        ...
        if len(list_message) < 1:
            embed = discord.Embed(title="OpenTTD 디스코드 봇 도우미", description='사용하실 커맨드를 작성해주세요.', color=self.color)
    ...
    async def reload(self, ctx):
        ...
        embed = discord.Embed(
            title="OpenTTD 디스코드 봇",
            description=f"{len(exts)}개의 command가 재로딩 되었습니다.",
            colour=self.color
        )
        if not e_msg == "":
